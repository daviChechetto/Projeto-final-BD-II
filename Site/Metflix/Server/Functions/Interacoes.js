...
router.post('/perfis/:id_perfil/minha-lista', authenticateToken, (req, res) => {
    ...
    try {
        const itemExistente = dbGet(
            'SELECT * FROM Minha_Lista WHERE id_perfil = ? AND id_conteudo = ?',
            [id_perfil, id_conteudo]
        );

        if (itemExistente) {
            dbRun('DELETE FROM Minha_Lista WHERE id_minha_lista = ?', [itemExistente.id_minha_lista]);
            res.status(200).json({ mensagem: 'Conteúdo removido da sua lista.' });
        } else {
            dbRun('INSERT INTO Minha_Lista (id_perfil, id_conteudo) VALUES (?, ?)', [id_perfil, id_conteudo]);
...
router.post('/perfis/:id_perfil/historico', authenticateToken, (req, res) => {
    ...
    try {
        ...
        const itemExistente = dbGet(sql_check, params_check);
...
// ... (O resto das rotas GET de Interacoes.js pode continuar igual, usando dbAll)
...
router.get('/perfis/:id_perfil/minha-lista', authenticateToken, (req, res) => {
    ...
    try {
        const lista = dbAll(sql, [id_perfil]);
